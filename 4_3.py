...
SPEED = 6               # km/day 
BETA1 = 0 #0.50            # Probality to gets infected (From "S" to "I") 
BETA2 = 1 #0.75            # Probability to get infected if you are part of an infected cluster
...
def voisin_infected(self): #voisin infect̩?
    for i in self.succ:
        if i.state == SIRState.INFECTIOUS:
            return True
    return False
...
class InfectiousPerson(Person):
    ...
    def update(self):
        ...
        elif self.age >= GAMMA1 : #si plus que 7 jours
...
def DFS(p,x,label):
    p[x].status = label
    for y in range(len(p[x].succ)):     #Modif
...
def no_respect_person (p,n): #p=people, n=len(people)
    ...
    for i in range(n):
        ...
        if p[i].state ==SIRState.INFECTIOUS:
...
def create_data_test_4_3():
    S = [ SusceptiblePerson(0.5, 0.25) for i in range(10) ]
    I = [ InfectiousPerson(0.5, 0.25) for i in range(10) ]
    return S + I   

import matplotlib.animation as animation
people = create_data_test_4_3()
